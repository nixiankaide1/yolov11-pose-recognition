...
这个项目是Ultralytics YOLO的WebUI界面,使用Gradio框架构建。它允许用户通过图形界面来训练、验证、预测和导出YOLO模型。

## 安装

1. 克隆此仓库:
   ```
   git clone https://github.com/your-username/ultralytics-yolo-webui.git
   cd ultralytics-yolo-webui
   ```
...
然后在浏览器中打开显示的URL(通常是 http://localhost:7860)。
